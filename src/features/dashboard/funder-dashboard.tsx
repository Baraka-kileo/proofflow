...
export function FunderDashboard({ data }: { data: FunderData }) {
  ...
  return (
    <div className="page-enter mx-auto max-w-6xl">
      <PageHeading
        eyebrow="Funder overview"
        title={count ? `${count} ${count === 1 ? "application is" : "applications are"} ready.` : "No applications are waiting."}
...
export function FunderOffersView({ data }: { data: FunderData }) {
  return (
    <div className="page-enter mx-auto max-w-5xl">
      <PageHeading eyebrow="Funder" title="Offers" description="Your recorded simulated offers and declines." />
      <Card className="mt-8"><CardContent className="pt-6"><div className="flex items-center justify-between gap-4"><h2 className="text-xl font-bold">Decision history</h2><StatusBadge status="neutral">{data.offers.length} total</StatusBadge></div>{data.offers.length ? <FunderOfferList data={data} /> : <EmptyState title="No decisions yet" description="Decisions will appear after evidence review." />}</CardContent></Card>
      <p className="mt-4 text-xs text-[var(--muted)]">All offers and funding actions are hackathon simulations.</p>
    </div>
  );
}
...
function TaskCard({ title, description, count, href, action }: { title: string; description: string; count: string; href: string; action: string }) {
  return <Card><CardContent className="pt-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">{title}</h2><p className="mt-2 text-sm text-[var(--muted)]">{description}</p></div><StatusBadge status="neutral">{count}</StatusBadge></div><Link href={href} className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-[var(--primary)] px-4 text-sm font-bold text-white hover:bg-[var(--primary-pressed)]">{action}</Link></CardContent></Card>;
}
...
function FunderOfferList({ data }: { data: FunderData }) {
  return <div className="mt-6 divide-y divide-[var(--border)]">{data.offers.map((offer) => <article key={offer.id} className="grid gap-4 py-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-2"><b>{offer.supplier}</b><StatusBadge status={offer.kind === "offer" ? "success" : "error"}>{offer.kind === "offer" ? offer.status : "declined"}</StatusBadge></div><p className="mt-2 text-sm text-[var(--muted)]">{offer.invoice} · {offer.buyer} · Net {offer.net} · {offer.created}</p></div><QueueButton label="View decision" href={offer.href} /></article>)}</div>;
}
