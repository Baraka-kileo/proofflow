...
export function BuyerDashboard({ data }: { data: BuyerData }) {
  ...
  return (
    <div className="page-enter mx-auto max-w-6xl">
      <PageHeading
        eyebrow="Large customer overview"
        title={
          count
            ? `${count} ${count === 1 ? "confirmation" : "confirmations"} waiting.`
            : "You are up to date."
        }
      ...
      <section
        aria-label="Large customer tasks"
...
export function BuyerConfirmationsView({ data }: { data: BuyerData }) {
  ...
  return (
    <div className="page-enter mx-auto max-w-5xl">
      <PageHeading
        eyebrow="Large customer"
        title="Confirmations"
        description={
          count
            ? "Review one request at a time, starting with the oldest."
            : "You have no requests waiting."
        }
      />
      <Card className="mt-8">
        <CardContent className="pt-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <StatusBadge status={count ? "review" : "success"}>
              {count} pending
            </StatusBadge>
          </div>
          {count ? (
            <BuyerRequestList data={data} />
          ) : (
            <EmptyState
              title="Nothing to confirm"
              description="New confirmation requests will appear here."
            />
          )}
        </CardContent>
      </Card>
  ...
  return (
    <div className="page-enter mx-auto max-w-5xl">
      <PageHeading
        ...
        description="Completed confirmations and disputes are read-only."
      />
      <Card className="mt-8">
        <CardContent className="pt-6">
          <div className="flex items-center justify-between gap-4">
          ...
          {data.history.length ? (
            <BuyerHistoryList data={data} />
          ) : (
            <EmptyState
              title="No past decisions"
              description="Completed decisions will appear here."
            />
          )}
        </CardContent>
      </Card>
        ...
        >
          <div>
            <div className="flex flex-wrap gap-2">
              ...
              {request.warnings > 0 && (
                <StatusBadge status="review">
                  {request.warnings} {request.warnings === 1 ? "item" : "items"}
                </StatusBadge>
              )}
            </div>
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">
              {request.invoice} · {request.amount} · Waiting {request.age}
            </p>
          </div>
          <QueueButton
            label={
        ...
        >
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <b>{item.supplier}</b>
              <StatusBadge
                status={item.status === "confirmed" ? "success" : "error"}
              >
                {item.status}
              </StatusBadge>
            </div>
            <p className="mt-2 text-sm text-[var(--muted)]">
              {item.invoice} · {item.amount} · {item.decided}
            </p>
          </div>
          <QueueButton label="View receipt" href={item.href} />
        </article>
      ))}
    </div>
  );
