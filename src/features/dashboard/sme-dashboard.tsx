  ...
  return <div className="page-enter mx-auto max-w-6xl">
    ...
    <section className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_.7fr]">
      <Card><CardContent className="pt-6">{data.application?<>
        <div className="flex flex-wrap items-start justify-between gap-3"><div><span className="text-xs font-bold uppercase tracking-[.1em] text-[var(--primary)]">Current application</span><h2 className="mt-2 text-2xl font-bold tracking-tight">{data.application.buyer}</h2></div><StatusBadge status="success">{data.application.status}</StatusBadge></div>
        <div className="mt-7 grid grid-cols-2 gap-3 rounded-xl bg-[var(--soft)] p-4 text-sm"><span><small className="block text-[var(--muted)]">Reference</small><b>{data.application.id}</b></span><span><small className="block text-[var(--muted)]">Invoice value</small><b>{data.application.amount}</b></span></div>
        <Progress className="mt-7" label="Evidence journey" value={data.application.progress}/>
        <div className="mt-6 flex items-center justify-between gap-4"><p className="flex items-center gap-2 text-sm text-[var(--muted)]"><Clock3 className="size-4" />{data.application.next}</p><QueueButton label="View application"/></div>
