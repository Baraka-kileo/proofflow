...
import type { ApplicationDraftState } from "@/lib/validation/application-draft";

const initialState: ApplicationDraftState = { errors: [] };
...
export function ApplicationDraftForm({
  ...
  const error = (id: string) =>
    state.errors.find((item) => item.id === id)?.message;
  return (
    <form action={action} noValidate className="space-y-6">
      <ErrorSummary errors={state.errors} />
      {state.message && (
        <Alert tone="error" title="Draft not saved">
          {state.message}
        </Alert>
      )}
      <div className="grid gap-5 sm:grid-cols-2">
        <FormField
          ...
          label="Large customer organization"
          hint="The organization that will confirm the order and delivery."
          required
          error={error("buyerOrganizationId")}
        ...
        >
          <Select
            ...
            defaultValue=""
            aria-describedby={
              error("buyerOrganizationId")
                ? "buyerOrganizationId-error"
                : "buyerOrganizationId-help"
            }
            aria-invalid={Boolean(error("buyerOrganizationId"))}
          >
            <option value="" disabled>
              Select the large customer
            </option>
            {buyers.map((buyer) => (
        ...
        <FormField
          ...
          hint="Use the reference printed on the large customer's order or contract."
          required
          error={error("purchaseOrderReference")}
        >
          <Input
            id="purchaseOrderReference"
            ...
            aria-describedby={
              error("purchaseOrderReference")
                ? "purchaseOrderReference-error"
                : "purchaseOrderReference-help"
            }
            aria-invalid={Boolean(error("purchaseOrderReference"))}
          />
        </FormField>
        <FormField
          id="invoiceNumber"
          label="Invoice number"
          hint="This helps detect accidental duplicate applications."
          required
        ...
        <FormField
          ...
          hint="Enter the full invoice value, including cents."
          required
          error={error("invoiceAmount")}
        >
          <div className="grid grid-cols-[92px_1fr] gap-2">
            ...
            <Input
              ...
              name="invoiceAmount"
              inputMode="decimal"
              placeholder="48750.00"
              aria-describedby={
                error("invoiceAmount")
                  ? "invoiceAmount-error"
                  : "invoiceAmount-help"
              }
              aria-invalid={Boolean(error("invoiceAmount"))}
            />
          </div>
        </FormField>
        <FormField
          ...
          hint="When the large customer is expected to settle the invoice."
          required
          error={error("expectedDueDate")}
        >
          <Input
            id="expectedDueDate"
            ...
            min={minDate}
            aria-describedby={
              error("expectedDueDate")
                ? "expectedDueDate-error"
                : "expectedDueDate-help"
            }
            aria-invalid={Boolean(error("expectedDueDate"))}
          />
        </FormField>
      </div>
