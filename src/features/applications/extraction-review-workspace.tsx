...
import { submitApplicationReview } from "@/app/(protected)/applications/[applicationId]/extraction-actions";
...
export function ExtractionReviewWorkspace({
  ...
  const isDemoExtraction = documents.some(
    (document) => document.provider === "proofflow-demo-fixture",
  );
      ...
      Object.fromEntries(
        documents.flatMap((document) =>
          document.fields.map((field) => [
            field.id,
            inputValue(field.normalizedValue),
          ]),
        ),
      ),
    [documents],
  ...
  const [state, submitAction, pending] = useActionState(
    submitApplicationReview.bind(null, applicationId),
    { status: "idle" as const },
  );
  const reviewedFields = documents.flatMap((document) =>
    document.fields.map((field) => ({
      id: field.id,
      value:
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2
                id="review-fields-heading"
                className="mt-2 text-2xl font-bold tracking-[-.03em]"
              >
                Check every extracted field
              </h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">
                AI suggests candidate values only. Compare them with the signed
                private preview and correct anything uncertain.
              </p>
            </div>
            <span className="w-fit rounded-full bg-[var(--soft)] px-3 py-2 text-xs font-bold">
              {documents.reduce(
                (sum, document) => sum + document.fields.length,
                0,
              )}{" "}
              fields to review
            </span>
          ...
          {isDemoExtraction && (
            <div className="mt-5 rounded-xl border border-[#d7b45a] bg-[#fff8df] px-4 py-3 text-sm font-bold text-[#654900]">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-[var(--soft)] p-1 xl:hidden">
            <button
              ...
              className={cn(
                ...
                mobileView === "document" && "bg-[var(--surface)] shadow-sm",
              )}
            >
              Document
            </button>
            <button
              type="button"
              aria-pressed={mobileView === "fields"}
              onClick={() => setMobileView("fields")}
              className={cn(
                "min-h-11 rounded-lg text-sm font-semibold",
                mobileView === "fields" && "bg-[var(--surface)] shadow-sm",
              )}
            >
              Fields
            </button>
            ...
            >
              ...
              {selected.previewUrl ? (
                ...
                <div className="grid min-h-72 place-items-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--soft)] p-6 text-center text-sm text-[var(--muted)]">
                  Preview expired or unavailable. Reload this private page to
                  request a new signed preview.
                </div>
              )}
            </div>
            <div
              className={cn(
                "space-y-4",
                mobileView !== "fields" && "hidden xl:block",
              )}
            >
              {selected.fields.map((field) => {
                const original = inputValue(field.normalizedValue);
                const source = inputValue(field.sourceValue);
                const edited = values[field.id] !== original;
                const check =
                  field.normalizedValue === null ||
                  (field.confidenceBps ?? 0) < 8000;
                return (
                  <div
                    key={field.id}
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      {check ? (
                        <span className="flex items-center gap-1 rounded-full bg-[#fff2d4] px-2 py-1 text-[11px] font-bold text-[#795500]">
                          <AlertTriangle
                            className="size-3"
                            aria-hidden="true"
                          />
                    ...
                    <p className="mt-1 text-xs text-[var(--muted)]">
                      Source: {kindLabels[selected.kind]}
                      {field.sourceLabel
                        ? ` · ${field.sourceLabel}`
                        : " · No visible source"}
                    </p>
                    <p className="mt-1 text-[11px] text-[var(--muted)]">
                      Document text: {source || "Missing"}
                    </p>
                    {field.name === "receiver_or_signature_present" ? (
                      <select
                        id={`field-${field.id}`}
                    ...
                    {formatHints[field.name] && (
                      <p className="mt-2 text-[11px] text-[var(--muted)]">
                        Normalized format: {formatHints[field.name]}
                      </p>
                    )}
                    {edited && (
                      <p className="mt-2 flex items-center gap-1 text-xs font-bold text-[var(--primary)]">
                        <PencilLine className="size-3.5" aria-hidden="true" />
                        Edited by you · Original: {original || "Missing"}
                      </p>
                    )}
                  </div>
                );
          ...
          {state.status === "error" && (
            <Alert tone="error" title="Review not saved" className="mt-6">
              <p>{state.message}</p>
            </Alert>
          )}
          <form
          ...
          >
            <input
              type="hidden"
              name="reviewedFields"
              value={JSON.stringify(reviewedFields)}
            />
            <p className="text-xs leading-5 text-[var(--muted)]">
              Submitting records your identity and whether each AI candidate was
              accepted or corrected. This approval cannot be supplied by AI.
            </p>
            <Button type="submit" loading={pending}>
              {pending ? "Saving human review…" : "Save reviewed fields"}
            </Button>
          </form>
        </CardContent>
      </Card>
