...
export type DashboardData =
  ...
  | { role: "buyer"; metrics: Metric[]; requests: { id: string; reference: string; href: string; supplier: string; invoice: string; amount: string; age: string; warnings: number }[] }
  | { role: "funder"; metrics: Metric[]; applications: { id: string; href:string; supplier: string; buyer: string; amount: string; dueDate:string; confirmed: string; warnings: number; certificateHref:string|null }[] };
...
export async function getDashboard(session:SessionUser):Promise<DashboardData>{
  ...
  if(session.role==="sme"){
    ...
    return{role:"sme",metrics:[{label:"Evidence completion",value:current?`${state?.progress??0}%`:"0%",note:current?state?.next??"Continue the evidence journey":"Start the first application"},{label:"Verified value",value:formatMoney(confirmed.reduce((sum,row)=>sum+(row.invoice_total_minor??0),0)),note:`Across ${confirmed.length} buyer-confirmed ${confirmed.length===1?"invoice":"invoices"}`},{label:"Buyer confirmations",value:String(confirmed.length),note:"RLS-filtered to your organization"}],application:current&&state?{id:shortId("PF",current.id),href:`/applications/${current.id}`,buyer:current.buyer.name,amount:formatMoney(current.invoice_total_minor),status:state.label,progress:state.progress,next:state.next}:null};
  }
  if(session.role==="buyer"){
    const {data,error}=await supabase.from("confirmations").select("id,requested_at,application:applications!confirmations_application_id_fkey(id,invoice_number,invoice_total_minor,owner:organizations!applications_owner_organization_id_fkey(name))").eq("buyer_organization_id",session.organizationId).eq("status","pending").order("requested_at",{ascending:true});
    assertQuery(error,"buyer confirmations");const rows=data??[];const applicationIds=rows.map(row=>row.application.id);
    const {data:verificationRows,error:warningError}=applicationIds.length?await supabase.from("verification_runs").select("id,application_id,completed_at,verification_checks(result)").in("application_id",applicationIds).eq("status","completed").order("completed_at",{ascending:false}):{data:[],error:null};
    assertQuery(warningError,"buyer verification warnings");
    const requests=rows.map(row=>{const latest=(verificationRows??[]).find(run=>run.application_id===row.application.id);return{id:row.id,reference:shortId("CF",row.id),href:`/confirmations/${row.id}`,supplier:row.application.owner.name,invoice:row.application.invoice_number??"Invoice pending",amount:formatMoney(row.application.invoice_total_minor),age:elapsed(row.requested_at),warnings:(latest?.verification_checks??[]).filter(check=>check.result!=="pass").length}});
  ...
  const {data,error}=await supabase.from("applications").select("id,invoice_total_minor,invoice_due_on,owner:organizations!applications_owner_organization_id_fkey(name),buyer:organizations!applications_buyer_organization_id_fkey(name)").in("status",["buyer_confirmed","funder_review"]);
  assertQuery(error,"funder review queue");const rows=data??[];const ids=rows.map(row=>row.id);
  const [{data:checks,error:checksError},{data:confirmations,error:confirmationsError}]=ids.length?await Promise.all([supabase.from("verification_runs").select("id,application_id,completed_at,verification_checks(result)").in("application_id",ids).eq("status","completed").order("completed_at",{ascending:false}),supabase.from("confirmations").select("id,application_id,decided_at,approval_id").in("application_id",ids).eq("status","confirmed")]):[{data:[],error:null},{data:[],error:null}];
  assertQuery(checksError,"verification summaries");assertQuery(confirmationsError,"confirmation summaries");
  const applications=rows.map(row=>{const confirmation=confirmations?.find(item=>item.application_id===row.id);const latestRun=(checks??[]).find(item=>item.application_id===row.id);return{id:shortId("PF",row.id),href:`/applications/${row.id}`,supplier:row.owner.name,buyer:row.buyer.name,amount:formatMoney(row.invoice_total_minor),dueDate:row.invoice_due_on?new Intl.DateTimeFormat("en-ZA",{dateStyle:"medium",timeZone:"Africa/Johannesburg"}).format(new Date(`${row.invoice_due_on}T12:00:00+02:00`)):"Date unavailable",confirmed:elapsed(confirmation?.decided_at??null),warnings:(latestRun?.verification_checks??[]).filter(item=>item.result!=="pass").length,certificateHref:confirmation?.approval_id?`/confirmations/${confirmation.id}/certificate`:null}});
