...
export type DashboardData =
  ...
  | { role: "funder"; metrics: Metric[]; applications: { id: string; supplier: string; buyer: string; amount: string; confirmed: string; warnings: number; certificateHref:string|null }[] };
...
export async function getDashboard(session:SessionUser):Promise<DashboardData>{
  ...
  const applications=rows.map(row=>{const confirmation=confirmations?.find(item=>item.application_id===row.id);const latestRun=(checks??[]).find(item=>item.application_id===row.id);return{id:shortId("PF",row.id),supplier:row.owner.name,buyer:row.buyer.name,amount:formatMoney(row.invoice_total_minor),confirmed:elapsed(confirmation?.decided_at??null),warnings:(latestRun?.verification_checks??[]).filter(item=>item.result!=="pass").length,certificateHref:confirmation?.approval_id?`/confirmations/${confirmation.id}/certificate`:null}});
