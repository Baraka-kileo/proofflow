...
export default async function ApplicationPage({params}:{params:Promise<{applicationId:string}>}){
  ...
  const [{data:documents,error:documentsError},{data:buyer,error:buyerError},{data:verificationRuns,error:verificationError},{data:auditEvents,error:auditError},{data:confirmation,error:confirmationError}]=await Promise.all([
    ...
    supabase.from("confirmations").select("id,status,approval_id,reason").eq("application_id",application.id).maybeSingle(),
  ]);
  if(documentsError||buyerError||verificationError||auditError||confirmationError)throw new Error("Unable to load the private application workflow.");
