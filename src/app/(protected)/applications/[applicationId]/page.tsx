...
export default async function ApplicationPage({params}:{params:Promise<{applicationId:string}>}){
  const {application,session}=await requireApplicationAccess((await params).applicationId);
  const supabase=await createClient();
  const [{data:documents,error:documentsError},{data:buyer,error:buyerError},{data:owner,error:ownerError},{data:verificationRuns,error:verificationError},{data:auditEvents,error:auditError},{data:confirmation,error:confirmationError},{data:offer,error:offerError}]=await Promise.all([
    ...
    supabase.from("organizations").select("name").eq("id",application.owner_organization_id).maybeSingle(),
    ...
    supabase.from("confirmations").select("id,status,approval_id,reason,representative_name,representative_job_title,representative_email,decided_at").eq("application_id",application.id).maybeSingle(),
    supabase.from("offers").select("id,status,decision_kind,response_reason").eq("application_id",application.id).maybeSingle(),
  ]);
  if(documentsError||buyerError||ownerError||verificationError||auditError||confirmationError||offerError)throw new Error("Unable to load the private application workflow.");
  ...
  if(session.role==="funder"){
    const funderDocuments=await Promise.all((documents??[]).map(async document=>{const {data}=await supabase.storage.from("application-documents").createSignedUrl(document.storage_path,300);return{id:document.id,label:document.kind==="purchase_order"?"Purchase order":document.kind==="delivery_evidence"?"Delivery evidence":"Invoice",filename:document.original_filename,previewUrl:data?.signedUrl??null,fields:document.document_fields.map(field=>({label:field.field_name.replaceAll("_"," "),value:primitive(field.normalized_value)===null?"Missing":String(primitive(field.normalized_value))}))};}));
    return <div className="page-enter mx-auto max-w-7xl"><PageHeading eyebrow={`Funder review · ${application.id.slice(0,8).toUpperCase()}`} title={application.invoice_number??"Evidence package"} description="Review the source evidence, deterministic checks, and signed buyer confirmation before making your own simulated funding decision."/><div className="mt-8"><FunderApplicationReview applicationId={application.id} status={application.status} supplier={owner?.name??"Private supplier"} buyer={buyer?.name??"Private buyer"} invoice={application.invoice_number??"Invoice unavailable"} purchaseOrder={application.purchase_order_reference??"PO unavailable"} invoiceMinor={application.invoice_total_minor??0} currency={application.currency} dueDate={application.invoice_due_on??"1970-01-01"} documents={funderDocuments} checks={checks.map(check=>({code:check.ruleId,result:check.status,explanation:check.explanation}))} confirmation={confirmation?.status==="confirmed"&&confirmation.approval_id&&confirmation.representative_name&&confirmation.representative_job_title&&confirmation.representative_email&&confirmation.decided_at?{id:confirmation.id,name:confirmation.representative_name,role:confirmation.representative_job_title,email:confirmation.representative_email,decidedAt:confirmation.decided_at,approvalId:confirmation.approval_id}:null} audit={(auditEvents??[]).map(event=>({id:event.id,label:event.action.replaceAll("_"," ").replaceAll("."," · "),time:event.created_at}))}/></div></div>;
  }
  return <ApplicationTaskLayout {...progress}><div className="page-enter"><div className="flex flex-wrap items-center gap-2"><StatusBadge status="neutral">{application.status.replaceAll("_"," ")}</StatusBadge><span className="text-xs font-semibold text-[var(--muted)]">{buyer?.name??"Private buyer"} · {amount}</span></div><div className="mt-4"><PageHeading eyebrow={`Application · ${application.id.slice(0,8).toUpperCase()}`} title={application.invoice_number??"Private draft"} description="Your details are saved. Continue one evidence step at a time; progress is kept securely in your hosted workspace."/></div>{offer&&<Alert tone={offer.status==="offered"||offer.status==="accepted"?"success":"error"} title={offer.decision_kind==="decline"?"Funder declined this application":offer.status==="offered"?"Simulated offer ready":offer.status==="accepted"?"Simulated offer accepted":"Simulated offer declined"} className="mt-6"><p>{offer.decision_kind==="decline"?offer.response_reason:"Review the transparent terms and record your response."}</p><Link href={`/offers/${offer.id}`} className="mt-3 inline-flex min-h-11 items-center rounded-xl border border-[var(--border)] bg-white px-4 font-bold text-[var(--primary)]">View simulated offer</Link></Alert>}{confirmation?.status==="confirmed"&&confirmation.approval_id&&<Alert tone="success" title="Buyer confirmation completed" className="mt-6"><p>The signed confirmation is part of this application’s audit trail.</p><Link href={`/confirmations/${confirmation.id}/certificate`} target="_blank" className="mt-3 inline-flex min-h-11 items-center rounded-xl border border-[#9bc8b8] bg-white px-4 font-bold text-[var(--primary)]">View Confirmation Certificate</Link></Alert>}{confirmation?.status==="disputed"&&<Alert tone="error" title="Buyer disputed this receivable" className="mt-6"><p>{confirmation.reason??"Review the buyer confirmation record for details."}</p><Link href={`/confirmations/${confirmation.id}`} className="mt-3 inline-flex min-h-11 items-center rounded-xl border border-[#efc6c2] bg-white px-4 font-bold text-[var(--error)]">View buyer response</Link></Alert>}<div className="mt-8"><ApplicationDetailsReceipt buyer={buyer?.name??"Private buyer"} purchaseOrder={application.purchase_order_reference} amount={amount} dueDate={application.invoice_due_on} consented={Boolean(application.ai_processing_consented_at)}/></div>{currentStep==="review"&&application.status==="documents_uploaded"&&<div className="mt-8"><ReviewReadyPanel applicationId={application.id} extractionMode={extractionMode}/></div>}{currentStep==="review"&&application.status==="fields_extracted"&&reviewDocuments.length===3&&<div className="mt-8"><ExtractionReviewWorkspace applicationId={application.id} documents={reviewDocuments}/></div>}{showVerification&&<div className="mt-8"><VerificationPanel applicationId={application.id} status={application.status} overallResult={overallResult??null} checks={checks} completedAt={latestRun?.completed_at??null} sourceDocuments={sourceDocuments}/></div>}<div className="mt-8"><DocumentUploadPanel applicationId={application.id} status={application.status} documents={storedDocuments}/></div><div className="mt-8"><ApplicationActivity events={(auditEvents??[]).map(event=>({id:event.id,action:event.action,createdAt:event.created_at}))}/></div></div></ApplicationTaskLayout>;
}
