...
export async function createApplicationDraft(
  ...
  if (!parsed.success) {
    ...
    return {
      errors: [...new Map(errors.map((item) => [item.id, item])).values()],
    };
  }
  ...
  const { data: buyer, error: buyerError } = await supabase
    .from("organizations")
    .select("id")
    .eq("id", parsed.data.buyerOrganizationId)
    .eq("kind", "buyer")
    .maybeSingle();
  if (buyerError || !buyer)
    return {
      errors: [
        {
          ...
          message: "Choose an available large customer organization.",
        },
      ],
    };
  ...
  const { error } = await supabase.from("applications").insert({
    id: applicationId,
    owner_organization_id: session.organizationId,
    buyer_organization_id: buyer.id,
    created_by: session.id,
    title: `Invoice ${parsed.data.invoiceNumber} evidence`,
    purchase_order_reference: parsed.data.purchaseOrderReference,
    invoice_number: parsed.data.invoiceNumber,
    invoice_total_minor: amountMinor,
        ...
        errors: [
          {
            id: "invoiceNumber",
            message:
              "This invoice number already exists for the selected large customer.",
          },
        ],
      };
    return {
