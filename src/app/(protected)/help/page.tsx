...
const roles = [
  ...
  },
  {
    ...
    text: "Answers six transaction questions, corrects any disagreement, and signs a declaration tied to a permanent confirmation receipt.",
    icon: Building2,
  },
  {
    title: "Funder / Bank",
...
export default async function HelpPage() {
  ...
  return (
    <div className="page-enter mx-auto max-w-5xl">
      <PageHeading
        eyebrow="Help"
        title="One evidence trail, three clear roles."
        description="ProofFlow keeps each participant focused on the facts they are authorised to provide or review."
      />
      <section
        className="mt-10 grid gap-4 md:grid-cols-3"
        aria-label="ProofFlow roles"
      >
        {roles.map(({ title, text, icon: Icon }) => (
          <Card key={title}>
            <CardContent className="pt-6">
              <span className="grid size-11 place-items-center rounded-xl bg-[var(--soft)] text-[var(--primary)]">
      ...
      <Card className="mt-6">
        <CardContent className="pt-6">
          ...
          <ol className="mt-4 list-decimal space-y-3 pl-5 text-sm leading-6 text-[var(--muted)]">
            <li>Do not submit a decision you cannot support.</li>
            <li>
              Use the visible correction, retry, or go-back action on the
              current screen.
            </li>
            <li>
              If access is denied, return to Overview and confirm you entered
