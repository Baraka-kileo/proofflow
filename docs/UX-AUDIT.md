...
| `/dashboard` — buyer | Same protected shell geometry | Status says the buyer is up to date | Metrics and task cards link to separate Confirmations and History pages | Hosted query failure reaches the generic retry boundary | No SME/funder navigation or in-page hash navigation |
| `/dashboard` — funder | Same protected shell geometry | Status says no applications are waiting | Metrics and task cards link to separate Applications and Offers pages | Hosted query failure reaches the generic retry boundary | No buyer/SME navigation; finance labels say `Simulated` |
| `/applications` | Protected list skeleton | Concise role-specific empty state | SME sees its applications and Start action; funder sees buyer-confirmed packages | Hosted query failure reaches the generic retry boundary | Buyer is denied; role-specific rows remain RLS-filtered |
| `/confirmations` | Protected list skeleton | `Nothing to confirm` | Buyer opens one pending request at a time | Hosted query failure reaches the generic retry boundary | Buyer only; completed decisions are excluded |
| `/confirmations/history` | Protected list skeleton | `No past decisions` | Buyer opens read-only confirmation/dispute receipts | Hosted query failure reaches the generic retry boundary | Buyer only; no mutation controls |
| `/offers` | Protected list skeleton | `No decisions yet` | Funder opens recorded simulated offers and declines | Hosted query failure reaches the generic retry boundary | Funder only; simulation disclosure remains visible |
...
| `/confirmations/[id]` | Focused task skeleton | Missing/ineligible request fails closed | One numbered question at a time, conditional explanation, summary, declaration, drawing, and immutable receipt are complete | A `No` requires an explanation; Back retains answers; save failure is safe and retryable | Only pending buyer records can be decided; disputes bypass signature; completed records cannot change |
