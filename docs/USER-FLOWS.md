...
3. Confirm `order is ours`, `goods/services were received`, and `amount/reference are recognized`, or dispute with a required reason.
4. See an immutable confirmation receipt and history entry.

## Funder journey

1. Sign in to a queue showing buyer-confirmed packages first.
2. Open a package and see source documents, extracted facts, rule results, buyer attestation, duplicates, and audit timeline.
...
- Buyer owns confirmation/dispute.
