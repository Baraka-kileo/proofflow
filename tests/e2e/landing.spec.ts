...
test("each demo role receives a distinct dashboard in a separate browser session", async ({ browser }) => {
  const roles=[
    {button:"SME workspace Create and track evidence",workspace:"sme workspace",visibleNav:"Trust Passport",hiddenNav:"Offers"},
    {button:"Buyer workspace Confirm delivery requests",workspace:"buyer workspace",visibleNav:"History",hiddenNav:"Trust Passport"},
    {button:"Funder workspace Review simulated offers",workspace:"funder workspace",visibleNav:"Offers",hiddenNav:"Trust Passport"},
  ];
  for(const role of roles){const context=await browser.newContext();const page=await context.newPage();await page.goto("/login");await page.getByRole("button",{name:role.button,exact:true}).click();await expect(page.getByRole("heading",{level:1})).toBeVisible({timeout:15_000});await expect(page.getByText(role.workspace,{exact:true})).toBeVisible();await expect(page.getByRole("link",{name:role.visibleNav,exact:true}).first()).toBeVisible();await expect(page.getByRole("link",{name:role.hiddenNav,exact:true})).toHaveCount(0);await context.close();}
});
