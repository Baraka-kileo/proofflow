...
test("landing page renders its primary story without overflow", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Hackathon demo · no real money moves"),
  ).toBeVisible();
...
test("landing interactions expose focus and respect reduced motion", async ({
  page,
}) => {
  ...
  const presentation = await page.evaluate(() => {
    ...
    return {
      ...
      animationDuration: reveal
        ? getComputedStyle(reveal).animationDuration
        : "",
    };
  });

  expect(Number.parseFloat(presentation.outlineWidth)).toBeGreaterThan(0);
  expect(Number.parseFloat(presentation.animationDuration)).toBeLessThanOrEqual(
    0.01,
  );
});
...
test("protected shell is usable on mobile without overflow", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await enterDemoRole(page, "SME");
  await expect(
    page.getByRole("heading", { level: 1, name: "Good morning, Amara." }),
  ).toBeVisible();
  await expect(
...
for (const viewport of [
  ...
  test(`protected shell fits ${viewport.width}px without overflow`, async ({
    page,
  }) => {
    await page.setViewportSize(viewport);
    await enterDemoRole(page, "SME");
    const navigationName =
      viewport.width >= 1024 ? "Workspace navigation" : "Mobile navigation";
    await expect(
...
test("system states provide clear recovery", async ({ page, context }) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: "This page is not part of the evidence trail.",
    }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("alert").filter({ hasText: "You are offline" }),
  ).toContainText("You are offline");
  await context.setOffline(false);
});

test("demo login selects the buyer workspace through a hosted Supabase session", async ({
  page,
}) => {
  await page.goto("/login");
  await expect(
  ...
  expect(
    cookies.some(
      (item) => item.name.startsWith("sb-") && item.name.includes("auth-token"),
    ),
  ).toBe(true);
...
test("each demo role receives a distinct dashboard in a separate browser session", async ({
  browser,
}) => {
  const roles = [
    {
  ...
  for (const role of roles) {
    ...
    await expect(page.getByText(role.workspace, { exact: true })).toBeVisible();
    await expect(
      page.getByRole("link", { name: role.visibleNav, exact: true }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: role.hiddenNav, exact: true }),
    ).toHaveCount(0);
    await context.close();
  }
});
...
test("SME creates a validated private application draft", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Start with the invoice story." }),
  ).toBeVisible();
  ...
  await page
    .getByLabel(/Buyer organization/)
    .selectOption({ label: "Ubuntu Retail Group Demo" });
  ...
  await page
    .getByRole("checkbox", { name: /I understand and consent/ })
    .click();
  await page.getByRole("button", { name: /Create private draft/ }).click();
  ...
  await expect(progress).toContainText(
    "Upload all three evidence documents first.",
  );
  const invoiceSlot = page.getByTestId("document-slot-invoice");
  await invoiceSlot.getByLabel("Upload Invoice").setInputFiles({
    name: "notes.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("not evidence"),
  });
  await expect(invoiceSlot.getByRole("alert")).toContainText(
  ...
  const purchaseBytes = await uniquePdf(
    "output/pdf/proofflow-demo-purchase-order.pdf",
    "purchase-order-e2e",
  );
  const deliveryBytes = await uniquePdf(
    "output/pdf/proofflow-demo-delivery-evidence.pdf",
    "delivery-e2e",
  );
  const invoiceBytes = await uniquePdf(
    "output/pdf/proofflow-demo-invoice.pdf",
    "invoice-e2e",
  );
  await purchaseOrderSlot.getByLabel("Upload Purchase order").setInputFiles({
    name: "proofflow-demo-purchase-order.pdf",
    mimeType: "application/pdf",
    buffer: purchaseBytes,
  });
  await expect(purchaseOrderSlot).toContainText("1 page", { timeout: 25_000 });
  await deliverySlot.getByLabel("Upload Delivery evidence").setInputFiles({
    name: "renamed-delivery-proof.pdf",
    mimeType: "application/pdf",
    buffer: purchaseBytes,
  });
  await expect(deliverySlot.getByRole("alert")).toContainText(
    "V009 · Exact duplicate file",
    { timeout: 25_000 },
  );
  await expect(page.getByText("1 / 3 uploaded")).toBeVisible();
  await deliverySlot.getByLabel("Upload Delivery evidence").setInputFiles({
    name: "proofflow-demo-delivery-evidence.pdf",
    mimeType: "application/pdf",
    buffer: deliveryBytes,
  });
  await expect(deliverySlot).toContainText("1 page", { timeout: 25_000 });
  await invoiceSlot.getByLabel("Upload Invoice").setInputFiles({
    name: "proofflow-demo-invoice.pdf",
    mimeType: "application/pdf",
    buffer: invoiceBytes,
  });
  ...
  ).toContainText("2 of 5 complete");
  await expect(
    page.getByRole("heading", { name: "Documents are ready for extraction" }),
  ).toBeVisible();
  await expect(page.getByRole("status")).toHaveText(
    "Demo extraction—not processed by live AI",
  );
  await expect(
    invoiceSlot.getByRole("button", { name: "Preview" }),
  ).toBeVisible();
  await purchaseOrderSlot.getByRole("button", { name: "Remove" }).click();
  await expect(purchaseOrderSlot.getByText("Browse files")).toBeVisible({
    timeout: 15_000,
  });
  ...
  await expect(page.getByRole("heading", { name: invoice })).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Application information" }),
  ).toBeVisible();
  await expect(
    page.getByText("PO-E2E-DEMO-1042", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText("19 Oct 2099", { exact: true })).toBeVisible();
  await expect(page.getByText("2 / 3 uploaded")).toBeVisible();
  await expect(
    purchaseOrderSlot.getByText("Required", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("complementary", { name: "Application progress" }),
  ).toContainText("1 of 5 complete");
  await page.goto("/dashboard");
  await page.goBack();
  await expect(
    page.getByRole("complementary", { name: "Application progress" }),
  ...
  await page
    .getByRole("button", {
      name: "SME workspace Create and track evidence",
      exact: true,
    })
    .click();
  await expect(page).toHaveURL(/\/dashboard$/, { timeout: 15_000 });
  ...
  await expect(page.getByText("2 / 3 uploaded")).toBeVisible();
  await expect(
    page.getByText("proofflow-demo-delivery-evidence.pdf", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText("proofflow-demo-invoice.pdf", { exact: true }),
  ).toBeVisible();
  await expect(
    page
      .getByTestId("document-slot-purchase_order")
      .getByText("Required", { exact: true }),
  ).toBeVisible();
});

test("buyer receives generic denial for the SME draft route", async ({
  page,
}) => {
