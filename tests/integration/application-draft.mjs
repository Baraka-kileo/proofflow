...
const {error}=await client.from("applications").insert({id:draftId,owner_organization_id:membership.organization_id,buyer_organization_id:"22222222-2222-4222-8222-222222222222",created_by:auth.user.id,title:`Invoice ${invoice} evidence`,purchase_order_reference:"PO-INTEGRATION-1042",invoice_number:invoice,invoice_total_minor:4875025,requested_amount_minor:3900020,currency:"ZAR",invoice_due_on:"2099-10-19",status:"draft"});
if(error)throw error;
const {data,error:readError}=await client.from("applications").select("id,purchase_order_reference,invoice_total_minor").eq("id",draftId).single();
if(readError)throw readError;
if(data.purchase_order_reference!=="PO-INTEGRATION-1042"||data.invoice_total_minor!==4875025)throw new Error("Stored draft fields did not round-trip.");
const {error:deleteError}=await client.from("applications").delete().eq("id",draftId);
if(deleteError)throw deleteError;
console.log("PASS: SME created, read, and cleaned up a hosted application draft.");
